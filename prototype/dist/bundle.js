...
const Sex = { MALE: 'MALE', FEMALE: 'FEMALE', UNSPECIFIED: 'UNSPECIFIED' };
 ...
 */
function calorieTarget(maintenanceKcal, bmrKcal, kgPerWeek, sex) {
...
const PROTEIN_G_PER_KG_DEFICIT = 2.0;
const PROTEIN_G_PER_KG_MAINTENANCE = 1.8;
 ...
 */
function macros(targetKcal, goalWeightKg, inDeficit) {
...
const TdeeConfidence = { NONE: 'NONE', LOW: 'LOW', MEDIUM: 'MEDIUM', HIGH: 'HIGH' };
const ADAPTIVE_MIN_DAYS = 14;
const ADAPTIVE_FULL_DAYS = 28;
 ...
 */
function adaptiveTdee(formula, records, weightChangeKg) {
 ...
 */
function weightTrend(entries, smoothing = TREND_SMOOTHING) {
...
function weeklyRate(points, windowDays = 14) {
 ...
 */
function fittedChangeKg(entries) {
...
function projectGoalDate(result, goalKg, from) {
 ...
 */
function activityBurn(type, durationSeconds, distanceMetres, elevationGainMetres, weightKg) {
...
function paceSecondsPerKm(distanceMetres, durationSeconds) {
...
const STEP_GOAL_MIN = 3000;
const STEP_GOAL_MAX = 20000;
 ...
 */
function suggestStepGoal(history, currentGoal = null) {
 ...
 */
function expectedStepFraction(hour, minute) {
...
function stepStatus(steps, goal, hour, minute) {
...
function minutesToWalk(steps, stepsPerMinute = 110) {
...
function energyBalance({ date, targetKcal, consumedKcal, exerciseKcal, proteinTargetG, macros: totals, maintenanceKcal }) {
 ...
 */
function lastCompleteDays(todayISO, count = 7) {
  return Array.from({ length: count }, (_, i) => addDays(todayISO, -(count - i)));
}

function weeklySummary(balances) {
...
function calculateStreak(outcomes, today) {
...
const milestoneReached = streak => STREAK_MILESTONES.find(m => m === streak) ?? null;

// ---------------------------------------------------------------------------
// Training plan
// ---------------------------------------------------------------------------

const WEEKLY_INCREASE = 0.10;
const DELOAD_EVERY = 4;
const DELOAD_FACTOR = 0.70;
const STARTING_RUN_METRES = 2000;
const MAX_RUN_METRES = 30000;
const STARTING_HIKE_MINUTES = 60;
 ...
 */
function prescriptionsForWeek(template, weekStartISO, weekIndex, baseline) {
...
function adherence(prescriptions, completed, upToISO) {
...
function nextStrengthPrescription(lastSets, repRange, consecutiveFailures = 0, isLowerBody = false) {
...
const NudgePriority = { LOW: 0, DEFAULT: 1, HIGH: 2 };
 ...
 */
function evaluateNudges(state, settings) {
